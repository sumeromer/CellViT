...
class ExperimentCellViTCoNic(BaseExperiment):
        ...
        **kwargs,
        ...
        if backbone_type.lower() == "default":
            ...
            if pretrained_model is not None:
                ...
                cellvit_pretrained = torch.load(pretrained_model, weights_only=False)
        ...
        if backbone_type.lower() == "vit256":
            ...
            if pretrained_model is not None:
                ...
                cellvit_pretrained = torch.load(pretrained_model, weights_only=False, map_location="cpu")
                self.logger.info(model.load_state_dict(cellvit_pretrained, strict=True))
    ...
    def get_transforms(
        ...
        if "Downscale".lower() in transform_settings:
            ...
            if p > 0 and p <= 1:
                transform_list.append(
                    A.Downscale(p=p, scale_range=(scale, scale))
                )
        ...
        if "GaussNoise".lower() in transform_settings:
            ...
            if p > 0 and p <= 1:
                transform_list.append(A.GaussNoise(p=p, var_limit=(0, var_limit)))
