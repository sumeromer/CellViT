...
class MoNuSegInference:
    ...
    def __load_model(self) -> None:
        ...
        model_checkpoint = torch.load(self.model_path, weights_only=False, map_location="cpu")
    ...
    def calculate_step_metric(
        ...
        pred_binary_map = torch.argmax(predictions["nuclei_binary_map"], dim=1).long()
        target_binary_map = gt["nuclei_binary_map"].to(self.device).long()
    ...
    def calculate_step_metric_overlap(
        ...
        cell_dice = (
            dice_score(
                preds=torch.Tensor(pred_arr).to(self.device).long(),
                target=target_binary_map.long(),
                num_classes=2,
