...
class InferenceCellViT:
    ...
    def inference_step(
        ...
        # Generate plots before calculate_step_metric modifies the tensors
        if generate_plots:
            # Make copies for plotting since calculate_step_metric will modify the dicts
            import copy
            pred_dict_for_plot = copy.deepcopy(predictions.get_dict())
            gt_dict_for_plot = copy.deepcopy(gt.get_dict())
        ...
        if generate_plots:
            self.plot_results(
                imgs=imgs,
                predictions=pred_dict_for_plot,
                ground_truth=gt_dict_for_plot,
    ...
    def calculate_step_metric(
        ...
        for i in range(len(pred_tissue)):
            ...
            pred_binary_map = torch.argmax(predictions["nuclei_binary_map"][i], dim=0).long()
            target_binary_map = gt["nuclei_binary_map"][i].long()
            cell_dice = (
                dice_score(preds=pred_binary_map, target=target_binary_map, num_classes=2, include_background=False, input_format="index", aggregation_level="global")
                .detach()
                .cpu()
            )
    ...
    def plot_results(
        ...
        # Convert from channel-first (B, C, H, W) to channel-last (B, H, W, C) format where needed
        # All model outputs are channel-first format
        ...
        ground_truth["nuclei_binary_map"] = ground_truth["nuclei_binary_map"].permute(0, 2, 3, 1)
        ...
        # Take channel 1 (nuclei channel) to match the prediction format
        gt_sample_binary_map = (
            ground_truth["nuclei_binary_map"][:, :, :, 1].detach().cpu().numpy()
        )
